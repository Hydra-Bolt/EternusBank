class Hakoniwa:
    def __init__(self, filename) -> None:
        self.data_records:list[dict] = self.parseJSON(filename)
    @staticmethod
    def parseJSON(file):
        import json
        with open(file) as f:
    ...
    def find(self, dict_find:dict):
        ret_list = []
        for data in self.data_records:
            for diction in dict_find:
                if diction in data and dict_find[diction] == data[diction]:
                    ret_list.append(data)
        return ret_list
    def insert(self, dict_update):
        self.data_records.append(dict_update)
        self.updateDB()
        
    def updateDB(self):
        import json
        with open("DB.json", "w") as file:
            json.dump(self.data_records, file, indent=2)
