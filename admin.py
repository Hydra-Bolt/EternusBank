...
class Admin(AdminLogin, AdminWindow):
    ...
    def generateReport(self):
        ...
        self.dialog.setStyleSheet("font: 16pt \"Modern No. 20\";border-radius:5px;border:1px solid black;padding:6px")
        ...
        label = QtWidgets.QLabel("Select which account you would like the report of:", self.dialog)
        ...
        if account["Transactions"] != []:
            ...
            report.chapter_body(
                {"Transactions": "No Transactions to plot"}
            )
    ...
    def updateRecords(self, row, column):
        ...
        print(changed_value, changed_key)
        ...
        try:
            ...
            output["Accounts"][changed_key] = changed_value
    ...
    def openWindow(self):
        ...
        self.Window.show()
        self.fields = ["Full Name", "CNIC", "Account Number", "Account Type", "Card Number", "Phone Number", "Balance"]
    ...
    def configTable(self):
        ...
        for record in self.userAccountInfo.data_records:
            ...
            for account in accounts:
                ...
                self.details_table.setItem(current_index, 0, QtWidgets.QTableWidgetItem(name))
                self.details_table.setItem(current_index, 1, QtWidgets.QTableWidgetItem(cnic))
                self.details_table.setItem(current_index, 2, QtWidgets.QTableWidgetItem(account["Account Number"]))
                self.details_table.setItem(current_index, 3, QtWidgets.QTableWidgetItem(account["Account Type"]))
                self.details_table.setItem(current_index, 4, QtWidgets.QTableWidgetItem(account["Card Number"]))
                self.details_table.setItem(current_index, 5, QtWidgets.QTableWidgetItem(account["Account Name"]))
                self.details_table.setItem(current_index, 6, QtWidgets.QTableWidgetItem(str(account["Balance"])))
                current_index+=1
    def setupDB(self):
